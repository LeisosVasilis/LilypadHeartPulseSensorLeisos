...
void interruptSetup() {
  ...
  TCCR1C = 0x06; // DON'T FORCE COMPARE
...
ISR(TIMER1_OVF_vect) {                      // triggered when Timer2 counts to 124
  ...
  if (N > 250) {                                  // avoid high frequency noise
    if ( (Signal > thresh) && (Pulse == false) && (N > (IBI / 5) * 3) ) {
      ...
      digitalWrite(blinkPin6, HIGH);
      delay(30);
      digitalWrite(blinkPin6, LOW);


      digitalWrite(blinkPin7, HIGH);
      delay(30);
      digitalWrite(blinkPin7, LOW);
  ...
  if (Signal < thresh && Pulse == true) {  // when the values are going down, the beat is over
    digitalWrite(blinkPin3, LOW);           // turn off pin 3 LED
